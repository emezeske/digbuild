    ...
    fundamental_patch_( world_seed, position, Vector2i( WorldGenerator::REGION_SIZE, WorldGenerator::REGION_SIZE ), fundamental_features )
{
    ...
    const Vector3i trilinear_box_size( WorldGenerator::REGION_SIZE, TRILINEAR_BOX_HEIGHT, WorldGenerator::REGION_SIZE );
...
ChunkV WorldGenerator::generate_region( const Vector2i& position )
{
    ...
    for ( int x = 0; x < CHUNKS_PER_REGION_EDGE; ++x )
    {
        ...
        {
            generate_chunk_column( region_features, position, Vector2i( x, z ), chunks );
        }
    }
}
...
)
{
    for ( int x = 0; x < Chunk::CHUNK_SIZE; ++x )
    {
        ...
        {
            const Vector2i relative_position = column_index * Chunk::CHUNK_SIZE + Vector2i( x, z );

            const Scalar fundamental_height =
                features.get_fundamental_patch().interpolate( vector_cast<Vector2f>( relative_position ) / Scalar( REGION_SIZE ) );

            const BicubicPatch octave_patch = features.get_octave_patch( relative_position / BICUBIC_OCTAVE_EDGE );

            const Vector2f octave_position(
                Scalar( x % BICUBIC_OCTAVE_EDGE ) / BICUBIC_OCTAVE_EDGE,
                Scalar( z % BICUBIC_OCTAVE_EDGE ) / BICUBIC_OCTAVE_EDGE
            ;)
            ...
            const int num_layers = sizeof( layers ) / sizeof( std::pair<Scalar, BlockMaterial> );

            unsigned bottom = 0;
            const Vector2i column_position = region_position + relative_position;
            ChunkSP chunk;
            ...
            for ( int i = 0; i < num_layers; ++i )
            {
                ...
                for ( int y = bottom; y <= top; ++y )
                {
                    ...
                    const Vector3f box_position(
                        Scalar( relative_position[0] ) / Scalar( trilinear_box_size[0] ),
                        Scalar( y                    ) / Scalar( trilinear_box_size[1] ),
                        Scalar( relative_position[1] ) / Scalar( trilinear_box_size[2] )
                    );
                    ...
                    {
                        chunk.reset( new Chunk( Vector3i( column_position[0], y / Chunk::CHUNK_SIZE, column_position[1] ) ) );
